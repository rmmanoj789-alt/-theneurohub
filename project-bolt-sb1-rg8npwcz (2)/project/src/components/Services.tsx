  ...
  return (
    <section id="conditions" className="py-20 md:py-28 gradient-bg">
      <div className="max-w-7xl mx-auto section-padding">
        ...
        {/* Category Tabs */}
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-10 overflow-x-auto pb-2 -mx-2 px-2 sm:overflow-visible sm:pb-0 sm:mx-0 sm:px-0">
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setActiveId(activeId === cat.id ? null : cat.id)}
              className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all border whitespace-nowrap ${
                activeId === cat.id
